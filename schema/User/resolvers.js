...
export default {
  Query: {
    ...
    fetchUserAppointments: async (_, { first, skip }, { prisma, user }) => {
      ...
      const output = await prisma.user({ id: user.id }).$fragment(UserAppointments(first, skip))
  ...
  Mutation: {
    reviewService: async (
      ...
      const createdReview = await prisma.review({ id: output.id }).$fragment(reviewToService)
    ...
    register: async (
      ...
      jwt.sign({ userEmail: email }, process.env.JWT_EMAIL_SECRET, (_, emailToken) => {
